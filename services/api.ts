  ...
  signIn: async (email: string, password: string) => {
    try {
      ...
      if (!error.response) {
        ...
        console.error("2. If using device/emulator, use your machine's IP instead of localhost");
        console.error(`   Current API URL: ${API_URL}`);
        throw new Error(`Cannot connect to server at ${API_URL}. Make sure the backend is running and accessible.`);
      }
      throw error;
    }
  },
